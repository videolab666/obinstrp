				    ...
				    bool require_timestamp, uint64_t timestamp_ns, struct sr_replay_coverage_info *info)
{
	...
	/* Prefer the sync calibration saved with this session. Current OBS source
	 * settings are only a fallback for legacy sessions that predate the camera
	 * registry. */
	int64_t sync_offset_ns = 0;
	char camera_key[SR_CAMERA_STABLE_KEY_MAX] = {0};
	if (!sr_session_resolve_camera(session_dir, camera_name, camera_key, sizeof(camera_key), &sync_offset_ns) &&
